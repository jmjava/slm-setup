...
flowchart TB
  ...
  subgraph forbidden["Must not"]
    direction LR
    pub["Public internet<br/>no tunnels, no port-forward"]
    cloud["Cloud-hosted agents<br/>cannot see a private GPU"]
  end
...
flowchart TB
  ...
  subgraph hosts["One active Ollama host"]
    ...
    halo["Roadmap<br/>Halo-class AMD<br/>ssh -L + Ollama + ROCm"]
  end
