...
| `files` | array of `{ path, content }` | no | Snippets the premium agent chooses to send. Not a full-repo dump. The server refuses `.env` / private-key / token blobs, more than 12 files, or `max_tokens` above 4096 before calling Ollama. |
...
exits if that check fails; hostname and private LAN URLs still only warn.
